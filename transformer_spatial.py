...
#### prompt for GPT #####
##首先，保持其余的设定不变，每次model forward时去掉historical beamformer results，也即：现在假设在第t个step，我们仅仅输入  H_t 和 W_{t-1} 对应的embedding token去得到 W_t (W_next)，去掉（W_0,...,W_{t-2}). 其次，放弃现有的对 H矩阵和 W矩阵的embedding压缩。请这么进行 model forwarding:
##假设在第 t 个step，输入 H_t 与 W_{t-1}两个矩阵，首先将
...
class ChannelDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        basis_vectors_subset = basis_vectors[:self.subspace_dim].T
...
def train_beamforming_transformer(config):
    ...
    Args:
        config: BeamformerTransformerConfig instance with model parameters
    """
    dataset = ChannelDataset(num_samples=1000*config.batch_size, num_users=config.num_users, num_tx=config.num_tx, P=config.SNR_power, subspace_dim=config.subspace_dim)
